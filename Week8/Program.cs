...
//string[] weapon = { "Magic wand", "Plastic fork", "Wooden sword", "Lego brick" };
...
string[] weapon = File.ReadAllLines(Path.Combine(folderPath, weaponsFile));

string hero = GetRadomValueFromArray(heros);
string heroWeapon = GetRadomValueFromArray(weapon);
int heroHP = GetCharacterHP(hero);
int heroStrikeStrength = heroHP;
Console.WriteLine($"Today {hero} ({heroHP} HP) saves the day with {heroWeapon}!");

string villain = GetRadomValueFromArray(villains);
string villainWeapon = GetRadomValueFromArray(weapon);
...
static string GetRadomValueFromArray(string[] someArray)
{
